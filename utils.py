...
def parameter_file_read_ion(path):
    ...
    for i in range(len(lines)):
        ...
        if 'ion_type' in lines[i]:
            parameter_dict['ion_type'] = parameter_pick(lines[i])
        if 'p_value_threshold' in lines[i]:
            parameter_dict['p_value_threshold'] = parameter_pick(lines[i])
        if 'ion_relative_mode' in lines[i]:
            parameter_dict['ion_relative_mode'] = parameter_pick(lines[i])
        if 'psite_run' in lines[i]:
            parameter_dict['psite_run'] = parameter_pick(lines[i])
        if 'ion_rank_threshold' in lines[i]:
            parameter_dict['ion_rank_threshold'] = parameter_pick(lines[i])
        if 'plabel_run' in lines[i]:
            parameter_dict['plabel_run'] = parameter_pick(lines[i])
        if 'explain_modification_min' in lines[i]:
            parameter_dict['explain_modification_min'] = int(
                parameter_pick(lines[i]))
        if 'explain_modification_max' in lines[i]:
            parameter_dict['explain_modification_max'] = int(
                parameter_pick(lines[i]))
        if 'ion_filter_mode' in lines[i]:
            parameter_dict['ion_filter_mode'] = int(parameter_pick(lines[i]))
...
def parameter_file_read(path):
    ...
    for i in range(len(lines)):
        ...
        if 'common_modification_list' in lines[i]:
            parameter_dict['common_modification_list'] = parameter_pick(
                lines[i])
        if 'mass_calibration' in lines[i]:
            parameter_dict['mass_calibration'] = parameter_pick(lines[i])
        if 'mass_of_diff_diff' in lines[i]:
            parameter_dict['mass_of_diff_diff'] = float(
                parameter_pick(lines[i]))
        if 'common_modification_number' in lines[i]:
            parameter_dict['common_modification_number'] = int(
                parameter_pick(lines[i]))
        if 'close_mass_diff_number' in lines[i]:
            parameter_dict['close_mass_diff_number'] = int(
                parameter_pick(lines[i]))
        if 'min_mass_modification' in lines[i]:
            parameter_dict['min_mass_modification'] = float(
                parameter_pick(lines[i]))
        if 'max_mass_modification' in lines[i]:
            parameter_dict['max_mass_modification'] = float(
                parameter_pick(lines[i]))
        if 'mass_diff_diff_range' in lines[i]:
            parameter_dict['mass_diff_diff_range'] = float(
                parameter_pick(lines[i]))
        if 'filter_frequency' in lines[i]:
            parameter_dict['filter_frequency'] = float(
                parameter_pick(lines[i]))
        ...
        if 'ion_labeling' in lines[i]:
            parameter_dict['ion_labeling'] = parameter_pick(lines[i])

        if 'ion_filter_ratio' in lines[i]:
            parameter_dict['ion_filter_ratio'] = float(
                parameter_pick(lines[i]))

        if 'ion_mass' in lines[i]:
            parameter_dict['ion_type'] = parameter_pick(lines[i])

        if 'p_value_threshold' in lines[i]:
            parameter_dict['p_value_threshold'] = float(
                parameter_pick(lines[i]))
    ...
    if parameter_dict['ion_labeling'] == 'True':
        parameter_dict['filter_frequency'] = 0.0
    return parameter_dict


def parameter_file_read_i(path):
    with open(path, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    parameter_dict = {}
    parameter_dict['psite_run'] = 'True'
...
def open_cfg_write(cfg_path, parameter_dict):
    ...
    for i in range(len(lines)):
        ...
        if 'activation_type' in lines[i]:
            lines[i] = parameter_modify(
                lines[i], parameter_dict['activation_type'])
        if 'modpath' in lines[i]:
            mod_path = os.path.join(
                parameter_dict['pfind_install_path'], 'bin')
        ...
        if 'maxdelta' in lines[i]:
            lines[i] = parameter_modify(lines[i], str(
                parameter_dict['max_mass_modification']))
        if 'msmsnum' in lines[i]:
            lines[i] = parameter_modify(
                lines[i], str(parameter_dict['msms_num']))
...
def close_cfg_write(cfg_path, current_path, parameter_dict, mass_diff_pair_rank):
    ...
    mod_line, flag = combine_common_list(
        current_path, parameter_dict, mass_diff_pair_rank)
    # print(mod_line)
    # 指定参数内容修改
    new_lines = []
    for i in range(len(lines)):
...
def modification_ini_generation(path, modification_dict):
    ...
    with open(new_ini_path, 'w', encoding='utf-8') as f:
        f.write('@NUMBER_MODIFICATION=' +
                str(len(common_modification_list)) + '\n')
...
def expand_modification_ini(mass_diff_pair_rank, mass_diff_dict, mod_static_dict, current_path, ini_path, parameter_dict, refine_ion_list=None, exist_ion_flag_list=None):
    ...
    for key in mass_diff_pair_rank:
        if parameter_dict['isotope_labeling'] == 'False':
            # key =  key[:-3] + "." + key[-2:]
            # print(key)
            # short_key = 'PFIND_DELTA_' + key.split('.')[0] + key.split('.')[1]
            # short_key = 'PFIND_DELTA_' + key[0:-2]
            short_key = 'PFIND_DELTA_' + key
        else:
            short_key = key.split('.')[0]
        ...
        if parameter_dict['isotope_labeling'] == 'False':
            ...
            key = key
        # pyp：并非所有的都在mass_diff_dict中
        if key not in mass_diff_dict.keys():
            continue
        ...
        if exist_ion_flag_list is not None and exist_ion_flag_list[ion_num//2] == True and ion_num < len(refine_ion_list) * 2 and len(refine_ion_list[ion_num//2][0]) >= 1:
            if ion_num % 2 == 0:
                line2 += ' 1 ' + str(refine_ion_list[ion_num//2][0][0]) + ' ' + str(
                    refine_ion_list[ion_num//2][0][0]) + ' pFindDELTA \n'
            else:
                line2 += ' 1 ' + str(refine_ion_list[ion_num//2][1][0]) + ' ' + str(
                    refine_ion_list[ion_num//2][1][0]) + ' pFindDELTA \n'
            ion_num += 1
        else:
            line2 += ' 0 pFindDELTA \n'
        lines.append(line2)
