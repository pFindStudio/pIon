...
def blind_search(current_path):
    ...
    parameter_dict['output_path'] = os.path.join(
        original_output_path, 'source')
    ...
    if parameter_dict['open_flag'] == 'True':
        ...
        common_modification_list = modification_ini_generation(
            current_path, modification_dict)
    else:
        # 使用参数文件中的列表来生成
        common_modification_list = modification_ini_generation_from_param(
            current_path, modification_dict, parameter_dict)
    #common_modification_list = common_modification_list[:2]
    # print(common_modification_list)

    # 重新生成blind.cfg文件
    res_path = blind_cfg_write(
        blind_cfg_path, current_path, parameter_dict, common_modification_list)
    ...
    # 过滤小于200和复数修饰
    name2mass, mass_diff_list = small_delta_filter(
        mass_diff_list, parameter_dict)

    mod_static_dict, mod_number_dict = mass_static(
        blind_path, current_path, mass_diff_list, parameter_dict['side_position'], parameter_dict)  # 统计位点分布
    # print(mod_number_dict)
    # print(mod_static_dict)

    # 计算精确质量
    # system_correct={mean, median}, mod_correct={mean, median, weight}
    mass_diff_dict, sim_mod_number_dict,  mod_std_dict, mod_r_dict = mass_correct(
        current_path, blind_path, mass_diff_list, parameter_dict, system_correct='mean', mod_correct='mean')
    ...
    # 质量写入pFind鉴定结果文件
    accurate_mass_for_result_file(
        pfind_summary_path, mass_diff_dict, parameter_dict)
    ini_path = modification_ini_path(parameter_dict)

    if parameter_dict['isotope_labeling'] == 'False':

        filter_mod, mass_diff_dict, mod_static_dict, new_mod_number_dict = unify_summary_write(current_path, mod_static_dict, mod_number_dict, mod2pep, mass_diff_dict,
        ...
        new_ini_path = expand_modification_ini(
            filter_mod, mass_diff_dict, mod_static_dict, current_path, ini_path, parameter_dict)
        ...
        mass_diff_pair_rank = add_mass_list(
            mass_diff_pair_rank, parameter_dict)   # 加上重标的质量，然后展平
        mass_diff_pair_rank = new_mass_list_generate(
            mass_diff_pair_rank, mass_diff_list)  # 取交集
        if parameter_dict['close_mass_diff_number'] < len(mass_diff_pair_rank):
            mass_diff_pair_rank = mass_diff_pair_rank[:
                                                      parameter_dict['close_mass_diff_number']]
    # print(mass_diff_pair_rank)
        new_ini_path = expand_modification_ini(mass_diff_pair_rank, mass_diff_dict, mod_static_dict,
                                               current_path, ini_path, parameter_dict, refine_ion_list, exist_ion_flag_list)

    # 生成限定式参数文件
    close_cfg_path = os.path.join(os.path.join(
        os.path.join(current_path, 'bin'), 'template'), 'close.cfg')
    close_cfg_write(close_cfg_path, current_path,
                    parameter_dict, mass_diff_pair_rank)
    # print(mass_diff_pair_rank)
